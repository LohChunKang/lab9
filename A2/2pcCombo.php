<html lang="en">
<head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>KFC</title>
        <link rel="stylesheet" href="css/productstyle.css"type="text/css">
        <link href="https://fonts.googleapis.com/css2?family=Metal+Mania&display=swap" rel="stylesheet">
    </head>
    <body>
        <div id="wrapper">
            <div id="header">
                <div id="sidelogo">
                    <a><img src="image/logoKFC.png" alt=""></a>
                </div>
                <div id="subheader">
                    <div class="container">
                        <a href="../A3/reg.php">SIGN IN</a>
                        <a>CONTACT US</a>
                        <a href="../index.php">HOME</a>
                    </div>
                </div>
            </div>

            <div class="product">
                <h2 style="margin-left: 10px;">2-pc Combo</h2><br>
                <table border="2">
                    <tr>
                        <td><img src="image/chickenmeals1.webp" alt=""></td>
                        <td><p>RM 20.49
                            <br></br>
                            In your meal:
                            <ul>
                                <li><a>2 Original Recipe Chicken</a></li>
                                <li><a>1 Whipped Potato (R) (Subject to availability)</a></li>
                                <li><a>1 Coleslaw (R)</a></li>
                                <li><a>1 Pepsi Black (Can)</a></li>
                            </ul></p></td>
                    </tr>
                </table>
            </div>
        </div>

        <div id="footer">
        <div class="container">
        <div class="footer_sub">
            <h2>About Us</h2>
            <ul>
                <li><a>KFC Malaysia</a></li>
                <li><a>The Colonel's Story</a></li>
                <li><a>Milestone and Achievements</a></li>
                <li><a>CSR</a></li>
                <li><a>New</a></li>
                <li><a>Join Us</a></li>
                <li><a>Scam Alert</a></li>
            </ul>
        </div>
        <div class="footer_sub">
            <h2>Food</h2>
            <ul>
                <li><a>FAQ</a></li>
                <li><a>Terms and Conditions</a></li>
                <li><a>Quality Assurance</a></li>
                <li><a>Halal Policy</a></li>
                <li><a>Nutrition Facts</a></li>
            </ul>
        </div>
        
        <div class="footer_sub_2">
            <h2>Subscribe Us</h2>
                <input type="text" name="email" class="email" placeholder="Enter your e-mail">
                <input type="submit" name="submit" value="SUBSCRIBE" class="submit">
                <p class="sub">Get latest promotions and news.</p>
        </div>
    </div>
</div>
</div>
</body>
</html>